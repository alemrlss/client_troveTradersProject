...
function antiguedadesComponent({ posts, user }) {
  ...
  //Filtro
  const otrosPosts = dataPosts.filter((post) => post.category === "otros");
  ...
  const currentPosts = otrosPosts.slice(indexOfFirstPost, indexOfLastPost);

  const totalPages = Math.ceil(otrosPosts.length / postsPerPage);
