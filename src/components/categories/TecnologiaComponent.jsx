...
function antiguedadesComponent({ posts, user }) {
  ...
  //Filtro
  const tecnologiaPosts = dataPosts.filter(
    (post) => post.category === "tecnologia"
  );
  ...
  const currentPosts = tecnologiaPosts.slice(indexOfFirstPost, indexOfLastPost);

  const totalPages = Math.ceil(tecnologiaPosts.length / postsPerPage);
