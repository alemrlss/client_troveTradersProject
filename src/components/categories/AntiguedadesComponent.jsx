...
function antiguedadesComponent({ posts, user }) {
  ...
  //Filtro
  const antiguedadesPosts = dataPosts.filter(
    (post) => post.category === "antiguedades"
  );
  ...
  const currentPosts = antiguedadesPosts.slice(
    indexOfFirstPost,
    indexOfLastPost
  );

  const totalPages = Math.ceil(antiguedadesPosts.length / postsPerPage);
