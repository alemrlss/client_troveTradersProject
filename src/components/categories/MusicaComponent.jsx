...
function musicaComponent({ posts, user }) {
  const [dataPosts] = useState(posts);

  //Filtro
  const musicaPosts = dataPosts.filter((post) => post.category === "musica");
  ...
  const currentPosts = musicaPosts.slice(indexOfFirstPost, indexOfLastPost);

  const totalPages = Math.ceil(musicaPosts.length / postsPerPage);
        ...
        <div className="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8 bg-white animate-fade animate-duration-[600ms]">
          {currentPosts.length === 0 ? (
            ...
            currentPosts.map((post) => (
              <CardProduct post={post} key={post._id} />
            ))
          )}
        </div>
