...
function antiguedadesComponent({ posts, user }) {
  ...
  //Filtro
  const deportesPosts = dataPosts.filter(
    (post) => post.category === "deportes"
  );
  ...
  const currentPosts = deportesPosts.slice(indexOfFirstPost, indexOfLastPost);

  const totalPages = Math.ceil(deportesPosts.length / postsPerPage);
