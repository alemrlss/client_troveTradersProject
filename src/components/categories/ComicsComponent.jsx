...
function antiguedadesComponent({ posts, user }) {
  ...
  //Filtro
  const comicsPost = dataPosts.filter((post) => post.category === "comics");
  ...
  const currentPosts = comicsPost.slice(indexOfFirstPost, indexOfLastPost);

  const totalPages = Math.ceil(comicsPost.length / postsPerPage);
